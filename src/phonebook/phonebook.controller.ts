...
export class PhonebookController {
  ...
  @Patch('/:phonebookId')
  async updateOnePhonebook(
    @Param('phonebookId') phonebookId: string,
    @Body() phonebook: { phone: number; name: string },
  ) {
    return await this.__phonebook.updateOnePhonebookFromTheDatabase(
      phonebookId,
      phonebook,
    );
  }
  ...
  async deleteOnePhonebook(@Param('phonebookId') phonebookId: string) {
    return await this.__phonebook.deleteOnePhonebookFromTheDatabase(
      phonebookId,
    );
  }
}
